...
export async function installDomainOs(opts: {
  ...
  const row = opts.store.createInstall({
    ...
    dashboardStyle: opts.input.dashboardStyle ?? "console",
    site: {
      logoUrl: brand.logoUrl,
      primaryColor: brand.primaryColor,
      dashboardStyle: opts.input.dashboardStyle ?? "console",
      writeup: opts.input.site?.writeup,
      phone: opts.input.site?.phone,
      email: opts.input.site?.email,
      address: opts.input.site?.address,
    },
